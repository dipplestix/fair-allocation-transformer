...
def best_nash_welfare(valuation_matrix, num_segments=10, return_allocation=False):
    """
    Approximate log with piecewise linear function
    Runtime: Polynomial

    Args:
        valuation_matrix: (n_agents, m_items) valuation matrix
        num_segments: Number of segments for piecewise linear approximation
        return_allocation: If True, also return the optimal allocation matrix

    Returns:
        nash_welfare: The optimal Nash welfare value
        allocation (optional): The allocation matrix if return_allocation=True
    """
    ...
    for i in range(m):
        model.addConstr(u[i] == gp.quicksum(scaled_valuation[i,j] * x[i,j]
    ...
    if model.status == GRB.OPTIMAL:
        ...
        if return_allocation:
            allocation = np.array([[int(round(x[i,j].x)) for j in range(n)] for i in range(m)])
            return nash_welfare / 10, allocation  # Scale back
        return nash_welfare / 10  # Scale back
    else:
        print("No optimal solution found, using bruteforce fallback")
        if return_allocation:
            # Fallback: return bruteforce result with allocation
            return best_nash_welfare_bruteforce(valuation_matrix, return_allocation=True)
        return best_nash_welfare_bruteforce(valuation_matrix)


def best_nash_welfare_bruteforce(valuation_matrix, return_allocation=False):
    ...
    for allocation in generate_all_allocations(list(range(n)), m):
        ...
        if current_nash > best_nash:
            best_nash = current_nash
            if return_allocation:
                best_allocation = allocation_matrix.copy()

    if return_allocation:
        return best_nash, best_allocation.astype(int) if best_allocation is not None else np.zeros((m, n), dtype=int)
    return best_nash

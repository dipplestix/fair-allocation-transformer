...
def main():
    ...
    parser.add_argument('--num_items_list', type=str, nargs='+', default=None, help='List of specific item counts to evaluate (default: all .npz files in folder)')
    ...
    parser.add_argument('--model_config', type=str, default=None, help='Path to model config file (optional)')
    ...
    # if eval_type is 'model' check that model_config is provided
    ...
    if args.num_items_list is not None:
        args.num_items_list = [int(x) for x in args.num_items_list]
    ...
    for npz_file in filtered_files:
        ...
        run_evaluation(npz_file, 'evaluation_results', 'evaluation_results', args.batch_size, args.eval_type, args.model_config)

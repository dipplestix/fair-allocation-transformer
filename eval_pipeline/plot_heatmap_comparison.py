#!/usr/bin/env python3
"""
Plot comparison heatmaps showing (Model+EF1) - (MaxUtil+EF1) difference.
"""
...
def load_and_plot(data_file, output_dir='results'):
    ...
    ax.set_title('Utility % Difference: (Model+EF1) - (MaxUtil+EF1)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(output_path / 'heatmap_diff_utility.png', dpi=150)
    plt.close()
    print(f"Saved: {output_path / 'heatmap_diff_utility.png'}")
    ...
    ax.set_xlabel('m (items)', fontsize=12)
    ax.set_ylabel('n (agents)', fontsize=12)
    ax.set_title('Nash % Difference: (Model+EF1) - (MaxUtil+EF1)', fontsize=14)
    cbar = plt.colorbar(im, ax=ax)
    ...
    plt.tight_layout()
    plt.savefig(output_path / 'heatmap_diff_nash.png', dpi=150)
    plt.close()
    print(f"Saved: {output_path / 'heatmap_diff_nash.png'}")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--input', type=str, default='results/heatmap_comparison.json')

...
from model_assets.model_components import GLU, MHA
...
class FATransformer(nn.Module):
    def __init__(self, n, m, d_model: int, num_heads: int, num_output_layers: int = 1, dropout: float = 0.0, initial_temperature: float = 1.0, final_temperature: float = 0.01):
        ...
        self.agent_proj = nn.Sequential(nn.Conv2d(1, d_model, kernel_size=1), nn.GELU())
        self.item_proj = nn.Sequential(nn.Conv2d(1, d_model, kernel_size=1), nn.GELU())
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_agent = x
        x_agent = self.agent_proj(x_agent)
        x_agent = x_agent.mean(dim=-1).permute(0, 2, 1)

        x_item = x.permute(0, 1, 3, 2)
        x_item = self.item_proj(x_item) 
        x_item = x_item.mean(dim=-1).permute(0, 2, 1)
